...
- `russia-regions.json` - регионы Российской Федерации: **86 субъектов**
- `russia-cities.json` - населённые пункты со статусом города: **1205 городов**, включая регион каждого из них
...
Основной список: 1102 города — данные на 2023 год. В 2023 году статус города получили населённые пункты Колтуши Ленинградской области и Ачхой-Мартан Чеченской Республики.

Дополнение: 103 города Донецкой Народной Республики, Луганской Народной Республики и Запорожской области (данные по переименованиям 2024 года).

Численность населения указана по итогам переписи населения на 1 октября 2021 года.

## Как воспроизвести или дополнить данные

Набор формируется конвейером в `scripts/` и может быть переиспользован для повторного обогащения новыми городами.

1. **Конфигурация** — `scripts/enrichment-config.json`: список регионов и городов для добавления (включая значения переименований).
2. **Сбор данных** (нужен API-ключ DaData в переменной окружения `DADATA_TOKEN`):
   ```bash
   npm run enrich:fetch
   ```
   Результат: `dadata-fetch-results.json`, `dadata-renamed-results.json`.
3. **Сборка** записей из конфигурации и результатов DaData:
   ```bash
   npm run enrich:build
   ```
   Результат: `new-cities.json`, `new-regions.json`.
4. **Применение** к основным спискам (идемпотентно — при повторном запуске новые записи не дублируются, а обновляются):
   ```bash
   npm run enrich:apply
   ```
5. **Нормализация** признака `isDualName` (пересчёт по реальному количеству вхождений имени):
   ```bash
   npm run dual:normalize
   ```

Шаги 3–5 можно выполнить одной командой:

```bash
npm run enrich
```

Проверить склонения сгенерированных названий:

```bash
npm run namecase:check
```

Генератор склонений вынесен в переиспользуемые модули: `scripts/lib/namecase.js` (7 падежей, сохранение «ё») и `scripts/lib/translit.js` (translit-метки).
...
- Александровск - Луганская Народная
- Артёмовск - Красноярский край
- Артёмовск - Донецкая Народная
- Артёмовск - Луганская Народная
